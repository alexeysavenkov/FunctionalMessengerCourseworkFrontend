...
authView authModel =
  div []
    [ h1 [class "display-1"] [text "Messenger"]
    , h1 [class "display-4"] [text "Authorization"]
    , input [onInput (\x -> UpdateAuthModel {authModel|phone=x}), placeholder "Phone", class "form-control"] []
    , input [onInput (\x -> UpdateAuthModel {authModel|password=x}), placeholder "Password", type_ "password", class "form-control"] []
    , br [] []
    , button [onClick Register, class "form-control col-md-3"] [text "Register"]
    --, br [] []
    , button [onClick Login, class "form-control col-md-3"] [text "Login"]
